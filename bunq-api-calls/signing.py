...
def generate_rsa_key_pair():
    """Generate or load an RSA key pair for API signing.
...
def load_private_key(private_key_pem):
    ...
    Returns:
        RSAPrivateKey: Private key object
    """
...
def verify_response(response_body, server_signature, server_public_key_pem):
    """Verifies the signature of a response from the Bunq API.
    
    Args:
        response_body (str): The response body to verify
        server_signature (str): Base64 encoded signature from the server
    ...
    try:
        ...
        signature = base64.b64decode(server_signature)
